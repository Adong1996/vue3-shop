<template>
  <div class="shop-info" v-if="info" ref="BScrollInfo">
...
export default {
  ...
  setup() {
    ...
    const supportClasses = reactive(['activity-green', 'activity-red', 'activity-orange'])
    const BScrollInfo = ref(null)
    const picsUl = ref(null)
    onMounted(()=>{
      if (info.value) {
        new BScroll(BScrollInfo.value, {})
        new BScroll(picsUl.value, {
          scrollX: true, // 水平滑动
          click: true,
        })
      }
